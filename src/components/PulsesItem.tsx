...
function PulsesItem({ pulses }) {
  const [editing, setEditing] = useState(false)
   const [id, setId] = useState(pulses.id)
  const [name, setName] = useState(pulses.name)
 const [description, setDescription] = useState(pulses.description)
 const [imageUrl, setImageUrl] = useState(pulses.imageUrl)
 const [markdown,setMarkDown]=useState(pulses.markdown)
  const [showDetails, setShowDetails] = useState(false)
  const [status, setStatus] = useState(pulses.status)
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4">
      {editing ? (
        ...
        <>
          <div className="flex flex-col md:flex-row md:justify-between items-center realtive">
            <div
              style={{
                width: '80%',
              }}
