...
function VegetableItem({ vegetable }) {
  const [editing, setEditing] = useState(false)
    const [id, setId] = useState(vegetable.id)
  const [name, setName] = useState(vegetable.name)
 const [description, setDescription] = useState(vegetable.description)
 const [imageUrl, setImageUrl] = useState(vegetable.imageUrl)
 const [markdown, setMarkDown] = useState(vegetable.markdown)
  const [showDetails, setShowDetails] = useState(false)
    const [status, setStatus] = useState(vegetable.status)
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await fetch(
        `http://3.13.92.74:30009/master-data/admin/vegetable/id/${vegetable.id}`,
        {
          method: 'PUT',
          headers: {
