...
function ModuleComponent(props) {
  ...
  const [description, setDescription] = useState('')
    const [selectmoduleType, setModuleType] = useState('')
  const handleCancelClick = () => {
    props.showComponent(false)
  }
  const handleAddModule = async () => {
    console.log(selectmoduleType)
    if(selectmoduleType==="submodule")
    {
         setModuleType("SUB_MODULE")
    }
    console.log(selectmoduleType)
    const moduleData = {
      name: name,
    ...
    <>
      <div className="flex justify-between mt-6">
        <div style={{ width: '80%' }}>
          ...
          <h2 className="text-lg font-medium">
            <span className="font-bold">Module Type:</span>{' '}
            <p>
              <select
                value={selectmoduleType}
