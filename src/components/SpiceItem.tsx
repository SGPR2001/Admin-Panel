...
function SpiceItem({ spice }) {
  const [editing, setEditing] = useState(false)
   const [id, setId] = useState(spice.id)
  const [name, setName] = useState(spice.name)
  const [description, setDescription] = useState(spice.description)
  const [imageUrl, setImageUrl] = useState(spice.imageUrl)
  const [markdown, setMarkDown] = useState(spice.markdown)
  const [showDetails, setShowDetails] = useState(false)
    const [status, setStatus] = useState(spice.status)
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await fetch(
        `http://3.13.92.74:30009/master-data/admin/spice/id/${spice.id}`,
        {
          method: 'PUT',
          headers: {
