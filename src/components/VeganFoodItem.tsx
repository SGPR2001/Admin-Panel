...
function VeganFoodItem({ veganfood }) {
  const [editing, setEditing] = useState(false)
   const [id, setId] = useState(veganfood.id)
  const [name, setName] = useState(veganfood.name)
  const [description, setDescription] = useState(veganfood.description)
  const [imageUrl, setImageUrl] = useState(veganfood.imageUrl)
  const [markdown, setMarkDown] = useState(veganfood.markdown)
  const [showDetails, setShowDetails] = useState(false)
    const [status, setStatus] = useState(veganfood.status)
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await fetch(
        `http://3.13.92.74:30009/master-data/admin/vegan-food/id/${veganfood.id}`,
        {
          method: 'PUT',
          headers: {
