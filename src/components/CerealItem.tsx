...
function CerealItem({ cereal }) {
  const [editing, setEditing] = useState(false)
   const [id, setId] = useState(cereal.id)
  const [name, setName] = useState(cereal.name)
  const [description, setDescription] = useState(cereal.description)
  const [imageUrl, setImageUrl] = useState(cereal.imageUrl)
  const [markdown, setMarkDown] = useState(cereal.markdown)
  const [showDetails, setShowDetails] = useState(false)
    const [status, setStatus] = useState(cereal.status)
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await fetch(
        `http://3.13.92.74:30009/master-data/admin/cereal/id/${cereal.id}`,
        {
          method: 'PUT',
          headers: {
