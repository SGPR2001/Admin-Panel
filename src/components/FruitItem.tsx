...
function FruitItem({ fruit }) {
  ...
  const [name, setName] = useState(fruit.name)
   ...
   const [imageUrl, setImageUrl] = useState(fruit.imageUrl)
   const [markdown, setMarkDown] = useState(fruit.markdown)
   const [showDetails, setShowDetails] = useState(false)
   const [status, setStatus] = useState(fruit.status)
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await fetch(
        `http://3.13.92.74:30009/master-data/admin/fruit/id/${fruit.id}`,
        {
          method: 'PUT',
          headers: {
