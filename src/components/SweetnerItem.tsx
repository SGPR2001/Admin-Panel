...
function SweetnerItem({ sweetner }) {
  const [editing, setEditing] = useState(false)
  const [name, setName] = useState(sweetner.name)
   const [id, setId] = useState(sweetner.id)
   const [description, setDescription] = useState(sweetner.description)
 const [imageUrl, setImageUrl] = useState(sweetner.imageUrl)
 const [markdown, setMarkDown] = useState(sweetner.markdown)
  const [showDetails, setShowDetails] = useState(false)
    const [status, setStatus] = useState(sweetner.status)
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await fetch(
        `http://3.13.92.74:30009/master-data/admin/sweetner/id/${sweetner.id}`,
        {
          method: 'PUT',
          headers: {
