...
function NonVegItem({ nonveg}) {
  const [editing, setEditing] = useState(false)
  const [name, setName] = useState(nonveg.name)
   const [id, setId] = useState(nonveg.id)
const [description, setDescription] = useState(nonveg.description)
const [imageUrl, setImageUrl] = useState(nonveg.imageUrl)
const [markdown, setMarkDown] = useState(nonveg.markdown)
const [status, setStatus] = useState(nonveg.status)
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await fetch(
        `http://3.13.92.74:30009/master-data/admin/meat/id/${nonveg.id}`,
        {
          method: 'PUT',
          headers: {
