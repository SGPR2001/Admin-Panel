...
function OilItem({ oil }) {
  const [editing, setEditing] = useState(false)
  const [name, setName] = useState(oil.name)
   const [id, setId] = useState(oil.id)
  const [description, setDescription] = useState(oil.description)
 const [imageUrl, setImageUrl] = useState(oil.imageUrl)
 const [markdown, setMarkDown] = useState(oil.markdown)
  const [showDetails, setShowDetails] = useState(false)
  const [status, setStatus] = useState(oil.status)
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await fetch(
        `http://3.13.92.74:30009/master-data/admin/oil/id/${oil.id}`,
        {
          method: 'PUT',
          headers: {
