...
function ClientQuestionComponent(props) {
const [name, setName] = useState('')
const [propertyCallbackUrl, setPropertyCallbackUrl] = useState('')
...
const handleAddModule = async () => {
    ...
    const moduleData = {
      name: name,
      propertyCallbackUrl: propertyCallbackUrl,
      method: method,
    }
    try {
    const response = await fetch(
        'http://3.13.92.74:30005/questionnaire/admin/client',
        {
        method: 'POST',
        headers: {
            'X-User-ID': '1',
...
return (
    ...
    <div className="flex justify-between mt-6">
        ...
        <h2 className="text-lg font-medium">
            ...
            <p className="border border-gray-400 p-1 rounded-sm">
            <input
                value={propertyCallbackUrl}
                onChange={(e) => setPropertyCallbackUrl(e.target.value)}
            />
            </p>
        </h2>
        <h2 className="text-lg font-medium">
