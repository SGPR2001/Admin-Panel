...
import { useSampleNonVeg } from '../hooks/sampleData'
import { NonVeg } from '../interfaces'
...
const TablesPage = () => {
  ...
  const handleFileInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const nonvegCsvFile = event.target.files?.[0]
    setSelectedFile(nonvegCsvFile)
  }

  const handleUploadButtonClick = async () => {
    if (selectedFile) {
      const formData = new FormData()
      formData.append('nonVegCsvFile', selectedFile)

      try {
        const response = await fetch(
          'http://3.13.92.74:30009/master-data/admin/meat/meat.csv',
          {
            method: 'POST',
            body: formData,
            headers: {
  ...
  const { nonveg } = useSampleNonVeg(pageNumber)
  ...
  const originalData = nonveg && nonveg.response ? nonveg.response : []
  const totalPages = nonveg && nonveg.totalPages ? nonveg.totalPages : 0
  ...
  const handleSearchClick = async () => {
    try {
      ...
      if (searchOption === 'id') {
        if (searchId) {
          url = `http://3.13.92.74:30009/master-data/admin/meat/id/${searchId}`
          ...
          if (response.ok) {
            const nonveg = await response.json()
            console.log(nonveg)
            setData([nonveg.response])
          } else {
        ...
        url = `http://3.13.92.74:30009/master-data/admin/meat/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
        ...
        url = `http://3.13.92.74:30009/master-data/admin/meat/regex/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
    ...
    <>
      <SectionMain>
        ...
        <CardBox className="mb-6">
          {searchId.length > 0
            ? data.map((nonveg: NonVeg) => (
                <CardBox key={nonveg.id} className="mb-6">
                  <NonVegItem key={nonveg.id} nonveg={nonveg} />
                </CardBox>
              ))
            : originalData.map((nonveg: NonVeg) => (
                <CardBox key={nonveg.id} className="mb-6">
                  <NonVegItem key={nonveg.id} nonveg={nonveg} />
                </CardBox>
              ))}
