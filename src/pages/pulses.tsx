...
import PulsesItem from '../components/PulsesItem'
import { useSamplePulses } from '../hooks/sampleData'
import { Pulses } from '../interfaces'
...
const TablesPage = () => {
 ...
 const handleReset = () => {
  ...
  const handleFileInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const pulsesCsvFile = event.target.files?.[0]
    setSelectedFile(pulsesCsvFile)
  }

  const handleUploadButtonClick = async () => {
    if (selectedFile) {
      const formData = new FormData()
      formData.append('pulsesCsvFile', selectedFile)

      try {
        const response = await fetch(
          'http://3.13.92.74:30009/master-data/admin/pulses/pulses.csv',
          {
            method: 'POST',
            body: formData,
            headers: {
  ...
  const { pulses } = useSamplePulses(pageNumber)
  ...
  const originalData = pulses && pulses.response ? pulses.response : []
  const totalPages = pulses && pulses.totalPages ? pulses.totalPages : 0
  ...
  const handleSearchClick = async () => {
    try {
      ...
      if (searchOption === 'id') {
        if (searchId) {
          url = `http://3.13.92.74:30009/master-data/admin/pulses/id/${searchId}`
        ...
        url = `http://3.13.92.74:30009/master-data/admin/pulses/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
        ...
        url = `http://3.13.92.74:30009/master-data/admin/pulses/regex/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
    ...
    <>
      <SectionMain>
        ...
        <CardBox className="mb-6">
          {searchId.length > 0
            ? data.map((pulses: Pulses) => (
                <CardBox key={pulses.id} className="mb-6">
                  <PulsesItem key={pulses.id} pulses={pulses} />
                </CardBox>
              ))
            : originalData.map((pulses: Pulses) => (
                <CardBox key={pulses.id} className="mb-6">
                  <PulsesItem key={pulses.id} pulses={pulses} />
                </CardBox>
              ))}
