...
const handleReset = () => {
  ...
  const handleFileInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const weekDietCsvFile = event.target.files?.[0]
    setSelectedFile(weekDietCsvFile)
  }

  const handleUploadButtonClick = async () => {
    if (selectedFile) {
      const formData = new FormData()
      formData.append('weekDietCsvFile', selectedFile)

      try {
        const response = await fetch(
          'http://3.13.92.74:30009/master-data/admin/week-diet/week-diet.csv',
          {
            method: 'POST',
            body: formData,
            headers: {
  ...
  const { weekdiet } = useSampleWeekDiet(pageNumber)
  ...
  const originalData = weekdiet && weekdiet.response ? weekdiet.response : []
  const totalPages = weekdiet && weekdiet.totalPages ? weekdiet.totalPages : 0
  ...
  const handleSearchClick = async () => {
    try {
      ...
      if (searchOption === 'id') {
        if (searchId) {
          url = `http://3.13.92.74:30009/master-data/admin/week-diet/id/${searchId}`
        ...
        url = `http://3.13.92.74:30009/master-data/admin/week-diet/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
        ...
        url = `http://3.13.92.74:30009/master-data/admin/week-diet/regex/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
    ...
    <>
      <SectionMain>
        ...
        <CardBox className="mb-6">
          {searchId.length > 0
            ? data.map((weekdiet: WeekDiet) => (
                <CardBox key={weekdiet.id} className="mb-6">
                  <WeekDietItem key={weekdiet.id} weekdiet={weekdiet} />
                </CardBox>
              ))
            : originalData.map((weekdiet: WeekDiet) => (
                <CardBox key={weekdiet.id}  className="mb-6">
                  <WeekDietItem key={weekdiet.id} weekdiet={weekdiet} />
                </CardBox>
              ))}
