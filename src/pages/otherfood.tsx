...
import OtherFoodItem from '../components/OtherFoodItem'
import { useSampleOtherFood } from '../hooks/sampleData'
import { OtherFood } from '../interfaces'
...
const handleReset = () => {
  ...
  const handleFileInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const otherFoodCsvFile = event.target.files?.[0]
    setSelectedFile(otherFoodCsvFile)
  }

  const handleUploadButtonClick = async () => {
    if (selectedFile) {
      const formData = new FormData()
      formData.append('otherFoodCsvFile', selectedFile)

      try {
        const response = await fetch(
          'http://3.13.92.74:30009/master-data/admin/other-food/other-food.csv',
          {
            method: 'POST',
            body: formData,
            headers: {
  ...
  const { otherfood } = useSampleOtherFood(pageNumber)
  ...
  const originalData = otherfood && otherfood.response ? otherfood.response : []
  const totalPages = otherfood && otherfood.totalPages ? otherfood.totalPages : 0
  ...
  const handleSearchClick = async () => {
    try {
      ...
      if (searchOption === 'id') {
        if (searchId) {
          url = `http://3.13.92.74:30009/master-data/admin/other-food/id/${searchId}`
          ...
          if (response.ok) {
            const otherfood = await response.json()
            console.log(otherfood)
            setData([otherfood.response])
          } else {
        ...
        url = `http://3.13.92.74:30009/master-data/admin/other-food/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
        ...
        url = `http://3.13.92.74:30009/master-data/admin/other-food/regex/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
    ...
    <>
      <SectionMain>
        ...
        <CardBox className="mb-6">
          {searchId.length > 0
            ? data.map((otherfood: OtherFood) => (
                <CardBox key={otherfood.id} className="mb-6">
                  <OtherFoodItem key={otherfood.id} otherfood={otherfood} />
                </CardBox>
              ))
            : originalData.map((otherfood: OtherFood) => (
                <CardBox key={otherfood.id} className="mb-6">
                  <OtherFoodItem key={otherfood.id} otherfood={otherfood} />
                </CardBox>
              ))}
