...
import { useSampleDairy } from '../hooks/sampleData'
import { Dairy } from '../interfaces'
...
const handleReset = () => {
  ...
  const handleFileInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const dairyCsvFile = event.target.files?.[0]
    setSelectedFile(dairyCsvFile)
  }

  const handleUploadButtonClick = async () => {
    if (selectedFile) {
      const formData = new FormData()
      formData.append('dairyCsvFile', selectedFile)

      try {
        const response = await fetch(
          'http://3.13.92.74:30009/master-data/admin/dairy/dairy.csv',
          {
            method: 'POST',
            body: formData,
            headers: {
  ...
  const { dairy } = useSampleDairy(pageNumber)
  ...
  const originalData = dairy && dairy.response ? dairy.response : []
  const totalPages = dairy && dairy.totalPages ? dairy.totalPages : 0
  ...
  const handleSearchClick = async () => {
    try {
      ...
      if (searchOption === 'id') {
        if (searchId) {
          url = `http://3.13.92.74:30009/master-data/admin/dairy/id/${searchId}`
        ...
        url = `http://3.13.92.74:30009/master-data/admin/dairy/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
        ...
        url = `http://3.13.92.74:30009/master-data/admin/dairy/regex/name/${searchId}`
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'X-USER-ID': '1',
    ...
    <>
      <SectionMain>
        ...
        <CardBox className="mb-6">
          {searchId.length > 0
            ? data.map((dairy: Dairy) => (
                <CardBox key={dairy.id} className="mb-6">
                  <DairyItem key={dairy.id} dairy={dairy} />
                </CardBox>
              ))
            : originalData.map((dairy: Dairy) => (
                <CardBox key={dairy.id} className="mb-6">
                  <DairyItem key={dairy.id} dairy={dairy} />
                </CardBox>
              ))}
